...
RCT_CUSTOM_VIEW_PROPERTY(localIdentifier, NSString, UIView) {
  ...
  [PHImageManager.defaultManager
   ...
   resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
     [self generateImagesWithAsset:asset withView:(VideoSeekbarPreviewView*)view];
   }];
}
...
- (void)generateImagesWithAsset:(AVAsset*)asset withView:(VideoSeekbarPreviewView*)view {
  ...
  [assetImageGenerator generateCGImagesAsynchronouslyForTimes:times completionHandler:^(CMTime requestedTime, CGImageRef  _Nullable cgImage, CMTime actualTime, AVAssetImageGeneratorResult result, NSError * _Nullable error) {
    if (error != nil) {
      [Debug logWithError:error];
      return;
    }
    if (!cgImage) {
      [Debug logWithFormat:@"AVAssetImageGenerator resultHandler was called, but no CGImage was generated. Result = %@", result, nil];
      return;
    }
    UIImage* image = [UIImage imageWithCGImage:cgImage scale:1.0 orientation:UIImageOrientationRight];
    NSUInteger index = [times indexOfObject:[NSValue valueWithCMTime:requestedTime]];
