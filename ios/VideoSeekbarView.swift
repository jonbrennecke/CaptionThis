...
class VideoSeekbarView: UIView {
  ...
  override func layoutSubviews() {
    ...
    let width = self.bounds.size.width / CGFloat(self.imageViews.count)
  ...
  private func generateImages(withAsset asset: AVAsset) {
    ...
    assetImageGenerator.generateCGImagesAsynchronously(forTimes: times) { (requestedTime, cgImage, actualTime, result, error) in
      if let error = error {
